...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-4 flex justify-end">
            <a href="{{ route('su.user.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition duration-300">
                Tambah User
            </a>
        </div>
                    ...
                    @foreach ($users as $user)
                        <tr class="border-t border-gray-200">
                            ...
                            <td class="px-4 py-2 text-center">
                                <!-- Tombol Edit -->
                                <a href="{{ route('su.user.edit', $user->user_id) }}" class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-1 px-3 rounded-md shadow-md transition duration-300 mr-2">
                                    Edit
                                </a>

                                <!-- Form Hapus -->
                                <form action="{{ route('su.user.destroy', $user->user_id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
