...
<div class="container mx-auto">
    ...
    <form action="{{ route('su.barang.update', $barang->br_kode) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="br_status" id="br_status" class="w-full border border-gray-300 rounded-md p-2" required>
                <option value="0" {{ $barang->br_status == 0 ? 'selected' : '' }}>Barang Dihapus</option>
                <option value="1" {{ $barang->br_status == 1 ? 'selected' : '' }}>Kondisi Baik</option>
                <option value="2" {{ $barang->br_status == 2 ? 'selected' : '' }}>Rusak, Bisa Diperbaiki</option>
                <option value="3" {{ $barang->br_status == 3 ? 'selected' : '' }}>Rusak, Tidak Bisa Digunakan</option>
            </select>
        </div>

        <div class="mb-4">
            <button type="submit" class="bg-teal-500 text-white px-4 py-2 rounded-md">Perbarui</button>
            <a href="{{ route('su.barang.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-md ml-2">Batal</a>
        </div>
    </form>
</div>
@endsection